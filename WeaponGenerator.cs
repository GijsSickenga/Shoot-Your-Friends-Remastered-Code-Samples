...
public class WeaponGenerator : MonoBehaviour
{
    ...
    /// <summary>
    /// Updates the gun UI behaviours of the player with the specified ID.
    /// </summary>
    /// <param name="playerID">The ID of the player to update the gun UI for.</param>
    private void UpdateGunUIBehaviours(int playerID)
    {
	...
	/// <summary>
	/// Updates the gun UI variables of the player with the specified ID.
	/// </summary>
	/// <param name="playerID">The ID of the player to update the gun UI for.</param>
	private void UpdateGunUIVariables(int playerID)
	{
        // Grab a reference to the player's gun stats UI.
        GunUIParent gunUI = gunUIParents[playerID];
	...
	public void GenerateNewBehaviours()
	{
        ...
        foreach (LocalPlayerController player in GameData.players)
        {
            if (player != null)
            {
                ...
                UpdateGunUIBehaviours(player.playerIndex);
    ...
    public void GenerateNewWeapons()
    {
        ...
        foreach (LocalPlayerController player in GameData.players)
		{
			if (player != null)
			{
				...
				// Update gun UI for current player to new values.
				UpdateGunUIVariables(player.playerIndex);
	...
	/// </summary>
	private List<BehaviourWeight> GetListOfPossibleBehaviours(float totalWeight)
	{
