...
public class GrenadeBehaviour : WeaponBehaviour
{
    public GrenadeBehaviour(int playerID, BehaviourWeight behaviourSettings, WeaponBehaviour nextBehaviour) : base(playerID, behaviourSettings, nextBehaviour)
    {
        ...
        childProjectileDamage = Mathf.CeilToInt(Stats._projectileDamage / Settings.LerpWeightInt() * 2);

        // Reduce damage of projectiles spawned from explosion.
        if (NextBehaviour != null)
        {
            NextBehaviour.Stats = new WeaponStats(Stats._projectileSpeed, childProjectileDamage, Stats._projectileColor);
        }
    ...
    public override void OnTriggered(Vector3 position, Vector3 direction, BehaviourProjectile projectile, Collider2D col = null)
    {
        ...
        float randomInitialAngle = Random.Range(0, 360);
        
        for (int i = 0; i < Settings.LerpWeightInt(); i++)
        {
            Vector3 newRotation = new Vector3(0, 0, rotationPerProjectile * i + randomInitialAngle);
            if (NextBehaviour != null)
            {
